import p5 from "p5";
export async function generateP5Cover() {
  function createp5sketch(p) {
    ...
    function centerCanvas() {
      ...
      let y = (p.windowHeight - p.height) / 2 + 35;
      cnv.position(x, y);
    }
    ...
    p.draw = function() {
      ...
      for (let i = 0; i < divX; i++) {
        for (let j = 0; j < divY; j++) {
          ...
          let colHere = p.color(col + 10, col + 15, col);
          colHere.setAlpha(90);
          p.fill(colHere);
      ...
      if (p.mouseIsPressed) {
        ...
        p.rect(
          p.int(p.mouseX / spanX) * spanX,
          p.int(p.mouseY / spanY) * spanY,
          spanX,
          spanY
        );
        p.noStroke();
        p.rect(
          p.int(p.mouseX / spanX) * spanX - 5,
          p.int(p.mouseY / spanY) * spanY - 5,
          spanX + 10,
          spanY + 10
        );
      }
